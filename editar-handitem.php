<?php
	...
	$getSelectedHobbaMember_Query = "SELECT * FROM handitems WHERE id = ?";
	...
	$usersUsername = $usersRow->handitem;
	$usersRank = $usersRow->categoria;
	$usersImg = $usersRow->imagen;
?>

		<div class="container">
			<div class="section">
				<h5 class="section-title">Editar handitem <?php echo $usersUsername; ?></h5>
				...
				<div class="row">
					<form action="" method="post" enctype="multipart/form-data">
						<div class="input-field col s12 m6">
							<?php
								if(isset($_POST['add-member'])) {
									$name = $_POST['name'];
									$nameError = '<p class="panel-error">Tienes que insertar un número de handitems</p>';
									checkEmptyInput($name, $nameError);
								}
							?>
							<input type="text" id="name" name="name" value="<?php echo $usersUsername; ?>">
							<label for="name" class="active">Número/nombre</label>
						</div>
						<div class="input col s12 m6">
							...
							<select name="rank">
								<?php
									$getAllHobbaRanks_Query = "SELECT categoria FROM handitems";
						...
						<div class="col s12">
							<?php
								...
								$mensajeLogs = "<strong>".$_SESSION['username']."</strong> editó un comando :handitem";
								...
								$logs->execute(array($mensajeLogs, $_SESSION['username']));	
									...
									$addMember_Query = "UPDATE handitems SET handitem = ?, imagen = ?, categoria = ? WHERE id = ?";
									$addMember = $db->prepare($addMember_Query);
 
									$addMember->execute(array($name, $file_path, $rank, $members_id));
									echo '<script>window.location="/admin/editar-handitem?id='.$members_id.'"</script>';
								}	else if(isset($_POST['add-member']) AND !empty($name) AND !empty($rank)) {
									$addMember_Query = "UPDATE handitems SET handitem = ?, categoria = ? WHERE id = ?";
									$addMember = $db->prepare($addMember_Query);
