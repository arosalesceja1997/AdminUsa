					...
					<div class="col s12">
						<?php
							if(!empty($newsTitle) AND !empty($newsCategoryRetrieved) AND !empty($newsContent) AND !empty($newsAuthor)) {
								...
								$mensajeLogs = "<strong>".$_SESSION['username']."</strong> aprobó la noticia ".$newsTitle."";
								...
								$updateNews_Query = "UPDATE noticias SET titulo = ?, categoria = ?, contenido = ?, autor = ?, aprobado = '1' WHERE id = ?";
								...
								echo '<script>window.location.href="/admin/verificar-noticia?id='.$news_id.'";</script>';
							}
						?>
						<button type="submit" class="btn waves-effect waves-light" name="edit-news">Aprobar</button>
						<a href="/admin/aprobar-noticia" class="btn waves-effect waves-light">Devolverme</a>
					</div>
				</form>
				</div>
			</div>
