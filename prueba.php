<?php
	...
	if(!isset($_SESSION['user_id']) OR $_SESSION['userAllowed'] != true) {
		header("Location: /");
	}	else if($_SESSION['user_rank'] != "Administradores" AND $_SESSION['user_rank'] != "Colaboradores" AND $_SESSION['user_rank'] != "Reporteros" AND $_SESSION['user_rank'] != "Coordinación") {
			...
			<div class="section">
				...
				<?php
					if(isset($_POST['search-user'])) {
						...
						echo '<script>window.location.href="/admin/prueba?user-lf='.$newsSearched.'";</script>';
					}
				?>
				...
				<table class="striped responsive-table">
					...
					<tbody>
						...
						if(!isset($_GET['user-lf'])) {
							$getAllNews_Query = "SELECT nombre FROM usuarios ORDER BY id DESC LIMIT 10";
						...
						if(isset($_GET['user-lf'])) {
							...
							$getSearchedNews_Query = "SELECT nombre FROM usuarios WHERE nombre = '$getSearchedNewsQuery'";
