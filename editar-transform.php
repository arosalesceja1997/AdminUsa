<?php
	...
	if(isset($_GET['id']) AND !empty($_GET['id'])) {
		...
		echo '<script>window.location="/admin/lista-transform"</script>';
	}

	include "inc/head.php";
	include "inc/nav.php";

	$getSelectedHobbaMember_Query = "SELECT * FROM transform WHERE id = ?";
	...
	$usersRank = $usersRow->transform;
	$usersImg = $usersRow->imagen;
?>

		<div class="container">
			<div class="section">
				<h5 class="section-title">Editar transform <?php echo $usersRank;?></h5>
					...
					<form action="" method="post" enctype="multipart/form-data">
						<div class="input-field col s12 m6">
							<?php
								if(isset($_POST['add-member'])) {
									$name = $_POST['name'];
									$nameError = '<p class="panel-error">Tienes que insertar un  número de transform</p>';
									checkEmptyInput($name, $nameError);
								}
							?>
							<input type="text" id="name" name="name" value="<?php echo $usersRank; ?>">
							<label for="name" class="active">Número de transform</label>
						</div>
								...
								if(isset($_POST['add-member'])) {
									if(isset($_FILES['member-img'])) {
										...
										if(isset($_FILES['member-img']['name']) AND !empty($_FILES['member-img']['name']) AND in_array($file_extn, $allowedFormat)) {
											$file_path = '../imagenes/transform/'.substr(md5(time()), 0, 10).'.'.$file_extn;
						...
						<div class="col s12">
							<?php
								...
								$mensajeLogs = "<strong>".$_SESSION['username']."</strong> editó un comando :transform";
								...
								$logs->execute(array($mensajeLogs, $_SESSION['username']));	
									...
									$addMember_Query = "UPDATE transform SET transform = ?, imagen = ? WHERE id = ?";
									$addMember = $db->prepare($addMember_Query);
 
									$addMember->execute(array($name, $file_path, $members_id));
									echo '<script>window.location="/admin/editar-transform?id='.$members_id.'"</script>';
